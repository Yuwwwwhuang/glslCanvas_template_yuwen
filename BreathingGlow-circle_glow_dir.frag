...
for (int index=0; index<7; ++index)    
{
    ...
    float weight = smoothstep(0.436,-0.024,uv.y); //uv[-1,1]
    float freq = 3.0+float(index)*0.184;
    ...
    float circle_dist = abs(dist-0.512)+noise;                                //光環大小
    ...
    gl_FragColor = vec4((result)*dir*vec3(1.000,0.896,0.925)*0.2,1.248); //*0.3降低整體亮度
