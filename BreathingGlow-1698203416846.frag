...
void main() {
    ...
    float dir= dot(point, (uv))+0.318;
    
    //亂數作用雲霧
    float fog= fbm(0.488*uv+vec2(-0.1*u_time, -0.02*u_time))*0.6+0.1;
    ...
    float noise = gnoise(uv_flip*16.)*0.02;  //frenquency
    ...
    gl_FragColor = vec4(glow_model+fog);
